 ...
 *   3. Actividad por hora (barras horizontales) + Destacados
 *   4. Desglose de movimientos: Entradas, Salidas, Compras, Créditos, Inventario
 *   5. Todas las ventas agrupadas por producto
 *   6. Resumen del turno
 *
...
function renderCierre(cierre) {
  ...
  return `
    <div class="panel">
      ...
      <div class="footnote" style="margin-top:8px;">Estimaci&oacute;n a partir de ventas y movimientos &mdash; no incluye fondo inicial ni requiere que se haya cerrado la caja.</div>
    </div>`;
}
...
/**
 * Cuadre real de caja — lo que el cajero contó físicamente al cerrar,
 * comparado contra el "Efectivo esperado" calculado en el Cierre.
 * diferencia = contado − esperado (negativo = faltante, positivo = sobrante).
 */
function renderCuadreReal(cuadreReal) {
  if (!cuadreReal || !cuadreReal.items || cuadreReal.items.length === 0) return '';

  const filas = cuadreReal.items.map(it => {
    const diferencia = (Number(it.contado) || 0) - (Number(it.esperado) || 0);
    const esFaltante = diferencia < 0;
    const esExacto = diferencia === 0;
    const etiqueta = esExacto ? 'Cuadrado' : (esFaltante ? 'Faltante' : 'Sobrante');
    const color = esExacto ? 'var(--primary-dark)' : (esFaltante ? 'var(--danger)' : 'var(--primary-dark)');
    const signo = diferencia > 0 ? '+ ' : (diferencia < 0 ? '&minus; ' : '');
    return `
        <tr>
          ...
          <td class="num" style="color:${color}; font-weight:700;">${etiqueta} &middot; ${signo}${formatMoney(Math.abs(diferencia))}</td>
        </tr>`;
  }).join('');

  const totalDiferencia = cuadreReal.items.reduce((acc, it) => acc + ((Number(it.contado) || 0) - (Number(it.esperado) || 0)), 0);
  const cantidadTexto = cuadreReal.items.length === 1 ? 'cierre' : 'cierres';
  const signoTotal = totalDiferencia > 0 ? '+ ' : (totalDiferencia < 0 ? '&minus; ' : '');

  return `
  <div class="section-title">Cuadre real de caja</div>
  <div class="table-wrap">
    <table>
      <thead>
        <tr>
          <th>Cierre</th><th>Cajero</th>
          <th class="num">Fondo inicial</th><th class="num">Esperado</th>
  ...
  <div class="footnote" style="margin-top:-10px; margin-bottom:16px;">
    ${cuadreReal.items.length} ${cantidadTexto} en este periodo &middot; diferencia total: ${signoTotal}${formatMoney(Math.abs(totalDiferencia))}
  </div>`;
}

function renderProductsTable(productos) {  const filas = productos.map((p, i) => {
...
const CSS = `
  ...
  *{ box-sizing:border-box; }
  ...
  .kpi-card{ flex:1 1 0; min-width:0; background:var(--card); border:1px solid var(--border); border-radius:8px; padding:9px 10px; break-inside:avoid; }
  ...
  .highlight-grid{ display:flex; flex-direction:column; gap:8px; margin-bottom:16px; }
  ...
  .row-2col{ display:flex; gap:14px; margin-bottom:16px; }
  .row-2col > .panel{ flex:1; min-width:0; }
  ...
  .hour-row-label{ flex:0 0 42px; font-size:8.3px; color:var(--muted); font-weight:600; white-space:nowrap; }
  .hour-row-track{ flex:1 1 auto; height:8px; background:var(--accent-soft); border-radius:5px; overflow:hidden; }
  .hour-row-fill{ height:100%; background:var(--primary); border-radius:5px; }
  .hour-row-value{ flex:0 0 60px; font-size:8.3px; font-weight:700; text-align:right; font-variant-numeric:tabular-nums; color:var(--ink); white-space:nowrap; }
 ...
 * @param {Object} [datos.cuadreReal]       { items:[{hora, cajero, fondoInicial, esperado, contado}] } — diferencia y etiqueta (Faltante/Sobrante/Cuadrado) se calculan solos
...
export function generarInformeTurnoHTML(datos) {
  const {
    turno, periodo, generadoTexto, kpis = [], cierre, destacados = [], cobros = [],
    horas = [], notaHoras, cuadreReal, entradas, salidas, compras, creditos, inventario,
...
<div class="sheet">
  ...
  <div class="header">
    ...
    <div class="header-right">
      ...
      <div class="header-shift-time">${escapeHtml(turno.horaInicio)} &ndash; ${escapeHtml(turno.horaFin)}</div>
  ...
  ${renderCuadreReal(cuadreReal)}

  <div class="row-2col">
    <div style="flex:1.35;">
      ${renderHourList(horas, notaHoras)}
    </div>
    <div class="highlight-grid" style="flex:0.65; margin-bottom:0;">
